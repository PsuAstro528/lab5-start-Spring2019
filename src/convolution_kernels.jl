...
struct GaussianConvolutionKernel{T} <: AbstractConvolutionKernel  where {T<:Number}
  ...
  function GaussianConvolutionKernel(width::T; limit_kernel_width::Number=limit_kernel_width_default) where {T<:Number}
      ...
      dist = Normal(zero(width), width)
      norm = 1.0/(cdf(dist,limit_kernel_width)-cdf(dist,-limit_kernel_width))
...
function (kernel::GaussianConvolutionKernel)(x::Number)
    abs(x) < kernel.limit_kernel_width ? kernel.norm*pdf(kernel.dist,x) : zero(x)
end

function (kernel::GaussianConvolutionKernel)(x::AbstractArray{T}) where T<:Number
    kernel.(x)
end
